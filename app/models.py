...
class User(UserMixin, db.Model):
    ...
    email: so.Mapped[str] = \
        so.mapped_column(sa.String(120), index=True, unique=True)
    ...
    about_me: so.Mapped[Optional[str]] = \
...
class Post(db.Model):
    ...
    user_id: so.Mapped[int] = \
        so.mapped_column(sa.ForeignKey(User.id), index=True)
